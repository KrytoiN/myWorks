    ...
    public partial class AdminPanel : Form
    {
        ...
        private void AdminPanel_Load(object sender, EventArgs e)
        {
            foreach (User user in MainForm.UserRepositories.Values)
                usersView.Rows.Add(user.Name, user.IsBlocked, user.HasPasswordRestriction);
            bool block = Convert.ToBoolean(usersView.SelectedRows[0].Cells[1].Value.ToString());
            bool restriction = Convert.ToBoolean(usersView.SelectedRows[0].Cells[2].Value.ToString());
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            ...
            if (userAdd.Text.Contains(" "))
            {
                MessageBox.Show("В имени пользователя не может быть пробелов", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        {
            if ( usersView.SelectedRows[0].Cells[0].Value?.ToString() != null)
                userDel.Text = usersView.SelectedRows[0].Cells[0].Value.ToString();
            else
                return;
            bool block = Convert.ToBoolean(usersView.SelectedRows[0].Cells[1].Value.ToString());
            bool restriction = Convert.ToBoolean(usersView.SelectedRows[0].Cells[2].Value.ToString());
        ...
        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
            if (checkBox1.Checked)
            {
                ...
                if (user == null)
                    return;
                usersView.SelectedRows[0].Cells[1].Value = true;
            ...
            else
            {
                ...
                if (user == null)
                    return;
                usersView.SelectedRows[0].Cells[1].Value = false;
